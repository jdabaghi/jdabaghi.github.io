...
%: Finite volumes/Backward Euler scheme
\begin{equation*}
\begin{split}
&S^n(\bU_h^n)=0\\
...
\end{frame}
 ...
 \STATE{\textbf{Initialization (semismooth Newton):} \; Choose an initial vector $\bU^{n,\textcolor{royalblue}{0}} \egaldef \bU^{n-1} \in \R^{3 \Nsp}$, ($\kk=0$)}\\
 \STATE{\textbf{Do}}\\
 \STATE{\quad$\kk = \kk+1$}\\
 \STATE {\quad Compute $\mathbb{A}^{n,\kk-1}  \in \R^{3 \Nsp, 3 \Nsp}$, \quad $\bB^{n,\kk-1} \in \R ^{3 \Nsp}$ \\
 ...
 \STATE{\quad \textbf{Initialization (linear solver):} Define $\bU^{n,\kk,\textcolor{burntorange}{0}} = \bU^{n,\kk-1}$, ($\textcolor{burntorange}{i=0}$) 
 as}
...
\end{minipage}
\end{frame}
%%%%%%% Numerical scheme Newton min adaptivity%%%%%%
%\begin{frame}
...
%%%% Numerical experiments GMRES adaptivity%%%%%
%\begin{frame}
...
%%%% SLIDE ACCURACY ADAPTIVE STRATEGY%%%%%
%\begin{frame}
%\frametitle{Accuracy $\gammalin = 10^{-6}, \gammaalg = 10^{-3}$}
%\textcolor{cadmiumgreen}{$t = 1.05 \times 10^5$ years} 
%\vspace{-0.2 cm}
%\begin{figure}
%\centering
